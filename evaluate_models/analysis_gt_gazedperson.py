...
for _, row in sig_results.iterrows():
 ...
 #                     color='0.8')
 #     xcen +=.2
ax.figure.savefig("gt_gazedperson_{}_humansCNN.png".format(error), dpi=300, bbox_inches='tight')
plt.close()
...
models = ['Head CNN', 'HeadBody Transformer', 'Head Transformer', 'Body Transformer']
...
''' PART III Visualize Estimation Vectors '''
...
''' PART V. Visualize Raw Estimation (see what's the variance in model estimation '''
# headbody transformer

data_path = 'data/GroundTruth_gazedperson'
files = glob.glob('{}/HeadBody*vectors*'.format(data_path)) # get all vector information
results = pd.DataFrame()
for f in files:
    df = pd.read_excel(f)
    df.columns = [x if 'est' not in x else '_'.join(x.split('_')[1:]) for x in df.columns ]
